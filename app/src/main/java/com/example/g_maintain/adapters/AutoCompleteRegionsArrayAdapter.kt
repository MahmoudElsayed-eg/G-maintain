...
class AutoCompleteRegionsArrayAdapter(context: Context) : ArrayAdapter<String>(context,android.R.layout.simple_dropdown_item_1line),Filterable {
        ...
        val mFilter = object : Filter() {
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                ...
                if (constraint != null) {
                    runBlocking {
                        mArrayList.clear()
                        mArrayList.addAll(MyDatabase(context).getRegionDao().getRegionsByNameList(constraint.toString()))
